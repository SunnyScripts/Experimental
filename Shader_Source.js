...
layout(location = 1) out vec4 biasDeltaSumAvg;
layout(location = 2) out vec4 previousDesiredDeltaSumAvg;

uniform sampler2D currentNeuronsTexture;
uniform sampler2D currentNeuronsDesiredTexture;
uniform sampler2D currentBiasDeltaTexture;
uniform sampler2D currentBiasDeltaSumCountTexture;
...
void main()
{
    float currentNeuronsIndex = gl_FragCoord.x +.5 + (gl_FragCoord.y-.5) * currentNeuronsTextureWidth;
    if(currentNeuronsIndex > currentNeuronsRealCount)
        discard;
    ...
    float currentBiasDelta = decodeFloat(texture2D(currentBiasDeltaTexture, gl_FragCoord.xy));
    float currentBiasDeltaSumCount = decodeFloat(texture2D(currentBiasDeltaSumCountTexture, gl_FragCoord.xy));
    
    biasDeltaSumAvg = encodeFloat((adjustedErrorFloat + currentBiasDelta) / currentBiasDeltaSumCount);
    ...
    float currentWeightDeltaSumCount = decodeFloat(texture2D(currentWeightDeltaSumCountTexture, gl_FragCoord.xy));
    previousDesiredDeltaSumAvg = encodeFloat((sum + currentWeightDelta) / currentWeightDeltaSumCount);
}`;
...
export let terrainVertex = `precision highp float;
        attribute vec2 vertexPosition;
